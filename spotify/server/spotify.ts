...
class SpotifyHandler {
  ...
  constructor() {
    ...
    this.ws = new WebSocketClient("ws://localhost:8081");
    this.getAccessToken();
  }
  ...
  async refreshAccessToken(): Promise<string | void> {
    ...
    try {
      const access_token = await this.ws.getToken();
      if (!access_token) {
        DeskThing.sendError("Failed to get access token from WebSocketClient");
        throw new Error("Failed to get access token from WebSocketClient");
      }
      DeskThing.sendLog("Access token refreshed!");
      DeskThing.saveData({ access_token: access_token as string });
   ...
   * @returns
   */
  async getAccessToken(): Promise<void | string> {
    try {
      const access_token = await this.ws.getToken();
      if (!access_token) {
        DeskThing.sendError("Failed to get access token from WebSocketClient");
      ...
      DeskThing.sendLog("Access token retrieved!");
      DeskThing.saveData({ access_token: access_token as string });
      return;
    } catch (error) {
      DeskThing.sendError("Error getting access token:" + error);
 ...
 */
  ...
  async analysis() {
    ...
    DeskThing.sendLog("Getting analysis data");
    DeskThing.send({
      app: "spotify",
      type: "analysis",
  ...
  async features() {
    ...
    const featuresData = await this.makeRequest("get", url);
    DeskThing.sendLog("Getting features data");
    DeskThing.send({
      app: "spotify",
      type: "features",
